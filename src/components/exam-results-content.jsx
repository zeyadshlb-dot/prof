            ...
            <Card>
                <CardHeader>
                    <CardTitle className="text-lg flex items-center gap-2">
                        سجل النتائج
                        <Badge variant="secondary" className="text-xs font-normal">{filteredResults.length}</Badge>
                    </CardTitle>
                    <CardDescription>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead onClick={() => handleSort('score')} className="cursor-pointer hover:bg-muted/50 transition-colors">
                                    <div className="flex items-center gap-1">
                                        الدرجة
                                        <ArrowUpDown className="h-3 w-3" />
                                    </div>
                                </TableHead>
                                <TableHead onClick={() => handleSort('score_percent')} className="cursor-pointer hover:bg-muted/50 transition-colors">
                                    <div className="flex items-center gap-1">
                                        النسبة %
                                        <ArrowUpDown className="h-3 w-3" />
                                    </div>
                                ...
                                <TableHead onClick={() => handleSort('created_at')} className="text-right cursor-pointer hover:bg-muted/50 transition-colors">
                        ...
                        <TableBody>
                            {filteredResults.length === 0 ? (
                                ...
                                filteredResults.map((result, index) => {
                                    const percent = Math.round((result.score / result.total_score) * 100) || 0
                                        ...
                                        <TableRow key={result.id}>
                                            ...
                                            <TableCell>
                                                <div className="font-bold font-mono">
                                                    {result.score} <span className="text-muted-foreground font-normal text-xs">/ {result.total_score}</span>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <div className="flex items-center gap-1 font-mono text-sm">
                                                    <Percent className="h-3 w-3 text-muted-foreground" />
                                                    <span className={percent >= 50 ? "text-emerald-600 font-bold" : "text-destructive font-bold"}>
                                                        {percent}%
                                                    </span>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                {result.is_passed ? (
                                                    <Badge variant="secondary" className="bg-emerald-500/15 text-emerald-600 hover:bg-emerald-500/25 border-emerald-500/20">
