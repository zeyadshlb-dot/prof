...
export function CourseDetailsContent({ course, initialChapters }) {
    ...
    const handleSaveChapter = async () => {
        ...
        let result
        if (editingChapter) {
            result = await updateChapter(editingChapter.id, chapterTitle, undefined, course.id)
        } else {
    ...
    const handleMoveChapter = async (index, direction) => {
        ...
        // Save to DB
        const result = await reorderChapters(newChapters.map(c => ({ id: c.id })), course.id)
        if (result.success) {
    ...
    const handleAddQuestion = () => {
        if (!currentQuestion.question_text || !currentQuestion.correct_answer) {
            toast({ title: "تنبيه", description: "يرجى إكمال بيانات السؤال واختيار الإجابة الصحيحة", variant: "destructive" })
            return
        }
        setLessonData({
            ...lessonData,
        ...
        toast({ title: "تم إضافة السؤال", description: `إجمالي الأسئلة: ${lessonData.questions.length + 1}` })
    }
    ...
    const handleSaveLesson = async () => {
        ...
        if (lessonData.type === 'exam' && lessonData.questions.length === 0) {
            toast({ title: "تنبيه", description: "يجب إضافة سؤال واحد على الأقل للامتحان", variant: "destructive" })
            return
        }
        ...
        if (editingLesson) {
            result = await updateLesson(editingLesson.id, { ...lessonData, chapter_id: selectedChapterId }, course.id)
        } else {
            result = await createLesson({ ...lessonData, chapter_id: selectedChapterId }, course.id)
        }
    ...
    const getLessonIcon = (type) => {
        switch (type) {
            ...
            case 'exam': return <FileText className="h-4 w-4 text-violet-500" />
    ...
    const getLessonTypeBadge = (type) => {
        switch (type) {
            case 'video': return <Badge variant="outline" className="text-[10px] bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:text-blue-400 border-blue-500/20">فيديو</Badge>
            case 'exam': return <Badge variant="outline" className="text-[10px] bg-violet-50 text-violet-600 dark:bg-violet-500/10 dark:text-violet-400 border-violet-500/20">امتحان</Badge>
            case 'pdf': return <Badge variant="outline" className="text-[10px] bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-400 border-red-500/20">PDF</Badge>
            default: return <Badge variant="outline" className="text-[10px]">ملف</Badge>
        }
    }
        ...
        <div className="flex flex-col gap-6 font-(family-name:--font-cairo)">
            ...
            <div className="flex items-center justify-between bg-card p-6 rounded-lg border shadow-sm">
                <div>
                    ...
                    <p className="text-muted-foreground">{course.description || "لا يوجد وصف"}</p>
                    <div className="flex gap-2 mt-4">
                        ...
                        <Badge variant="outline" className="gap-1">
                            ...
                            {chapters.reduce((sum, ch) => sum + (ch.lessons?.length || 0), 0)} درس
                        </Badge>
                    </div>
                </div>
            ...
            <div className="space-y-4">
                {chapters.length === 0 ? (
                    <div className="text-center py-12 border-2 border-dashed rounded-lg">
                        <p className="text-muted-foreground">لا يوجد محتوى بعد. ابدأ بإضافة فصل.</p>
                    </div>
                ) : (
                    <Accordion type="multiple" defaultValue={chapters.map(c => c.id)} className="space-y-4">
                                ...
                                <div className="flex items-center justify-between py-2">
                                    <AccordionTrigger className="hover:no-underline flex-1">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <Badge variant="secondary" className="text-[10px]">{chapter.lessons?.length || 0} درس</Badge>
                                        </div>
                                    </AccordionTrigger>
                                    <div className="flex items-center gap-1">
                                        ...
                                        <Button
                                            ...
                                            className="h-7 w-7"
                                            disabled={chIdx === 0}
                                            onClick={(e) => { e.stopPropagation(); handleMoveChapter(chIdx, 'up'); }}
                                            title="نقل للأعلى"
                                        >
                                        ...
                                        <Button
                                            ...
                                            onClick={(e) => { e.stopPropagation(); handleMoveChapter(chIdx, 'down'); }}
                                            title="نقل للأسفل"
                                        >
                                        ...
                                        <Button
                                            ...
                                            onClick={(e) => { e.stopPropagation(); openEditChapterDialog(chapter); }}
                                            title="تعديل الفصل"
                                        >
                                            <Pencil className="h-3.5 w-3.5" />
                                        </Button>
                                        {/* Add Lesson */}
                                        <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); openAddLessonDialog(chapter.id); }} className="gap-1 h-7 text-xs">
                                            <Plus className="h-3 w-3" />
                                            درس
                                        </Button>
                                        {/* Delete Chapter */}
                                        <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); handleDeleteChapter(chapter.id); }} className="h-7 w-7 text-destructive hover:bg-destructive/10">
                                ...
                                <AccordionContent className="pt-2 pb-4 space-y-2">
                                    {chapter.lessons && chapter.lessons.length > 0 ? (
                                        chapter.lessons.map((lesson, lIdx) => (
                                            <div key={lesson.id} className="flex items-center justify-between p-3 bg-muted/40 rounded-md border hover:bg-muted/60 transition-colors group">
                                                <div className="flex items-center gap-3">
                                                    <span className="text-[10px] font-mono text-muted-foreground w-4">{lIdx + 1}</span>
                                                    {getLessonIcon(lesson.type)}
                                                    <span className="font-medium">{lesson.title}</span>
                                                    {getLessonTypeBadge(lesson.type)}
                                                    {lesson.is_free_preview && <Badge variant="secondary" className="text-xs">مجاني</Badge>}
                                                ...
                                                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <Button
                                                        variant="ghost"
                                                        size="icon"
                                                        onClick={() => openEditLessonDialog(lesson, chapter.id)}
                                                        className="h-8 w-8 text-primary hover:bg-primary/10"
                                                        title="تعديل الدرس"
                                                    >
                                                        <Pencil className="h-4 w-4" />
                                                    </Button>
                                                    <Button variant="ghost" size="icon" onClick={() => handleDeleteLesson(lesson.id)} className="h-8 w-8 text-muted-foreground hover:text-destructive">
                                        ...
                                        <p className="text-sm text-muted-foreground text-center py-2">لا توجد دروس في هذا الفصل.</p>
                                    )}
                                </AccordionContent>
                            </AccordionItem>
                        ))}
                ...
                <DialogContent>
                    ...
                    <DialogFooter>
                        <Button onClick={handleSaveChapter} disabled={loading}>
                            {loading ? "جاري الحفظ..." : (editingChapter ? "تحديث الفصل" : "إضافة")}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={isLessonDialogOpen} onOpenChange={setIsLessonDialogOpen}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {editingLesson ? "عدّل بيانات الدرس الحالي." : "أضف فيديو يوتيوب أو امتحان تفاعلي أو ملف PDF."}
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid gap-4 py-4">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Select
                                    ...
                                    onValueChange={(val) => setLessonData({ ...lessonData, type: val, pdf_url: "", video_url: "" })}
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        <SelectItem value="video">
                                            <div className="flex items-center gap-2"><Video className="h-4 w-4 text-blue-500" /> فيديو (YouTube)</div>
                                        </SelectItem>
                                        <SelectItem value="exam">
                                            <div className="flex items-center gap-2"><FileText className="h-4 w-4 text-violet-500" /> امتحان (Quiz)</div>
                                        </SelectItem>
                        ...
                        <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-md border">
                            <Switch
                                ...
                                onCheckedChange={(checked) => setLessonData({ ...lessonData, is_free_preview: checked })}
                            />
                            <div>
                                <Label className="cursor-pointer">درس مجاني (Free Preview)</Label>
                                <p className="text-xs text-muted-foreground">يمكن للطلاب غير المشتركين مشاهدة هذا الدرس</p>
                            </div>
                        </div>
                        ...
                        {lessonData.type === 'video' && (
                            <div className="space-y-4 border p-4 rounded-md bg-muted/20">
                                <div className="space-y-2">
                                    ...
                                    />
                                    <p className="text-xs text-muted-foreground">سيتم استخراج ID الفيديو تلقائياً.</p>
                                </div>
                                <div className="space-y-2">
                                    <Label className="flex items-center gap-2">
                        ...
                        {lessonData.type === 'exam' && (
                            <div className="space-y-4 border p-4 rounded-md bg-muted/20">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="space-y-2 flex-1">
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => setLessonData({ ...lessonData, passing_score: e.target.value })}
                                        />
                                    </div>
                                </div>
                                ...
                                {lessonData.questions.length > 0 && (
                                    <div className="space-y-2">
                                        <h4 className="text-sm font-semibold text-muted-foreground">الأسئلة المضافة:</h4>
                                        {lessonData.questions.map((q, idx) => (
                                            <div key={idx} className="flex items-center justify-between p-2 bg-background rounded-md border text-sm">
                                                <span className="flex items-center gap-2">
                                                    ...
                                                    <span className="truncate max-w-[350px]">{q.question_text}</span>
                                                </span>
                                                <Button type="button" variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => handleRemoveQuestion(idx)}>
                                                    <X className="h-3 w-3" />
                                                </Button>
                                            </div>
                                        ))}
                                ...
                                <div className="border-t pt-4 mt-2">
                                    <h4 className="font-semibold mb-3 flex items-center justify-between">
                                        إضافة سؤال جديد
                                        <Badge variant="outline">{lessonData.questions.length} أسئلة مضافة</Badge>
                                    </h4>
                                        ...
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <Button
                                                    ...
                                                    size="sm"
                                                    onClick={async () => {
                                                        if (lessonData.pdf_url) await deleteFile(lessonData.pdf_url)
                    ...
                    <DialogFooter>
                        <Button onClick={handleSaveLesson} disabled={loading || uploading} className="w-full sm:w-auto">
