...
export async function getDashboardStats() {
  ...
  try {
    ...
    const { count: courseCount } = await supabase
      .from("courses")
      .select("*", { count: "exact", head: true });
    const { count: publishedCourseCount } = await supabase
      .from("courses")
      .select("*", { count: "exact", head: true })
    ...
    const { count: lessonCount } = await supabase
      .from("lessons")
      .select("*", { count: "exact", head: true });
    const { count: examResultCount } = await supabase
      .from("exam_results")
      .select("*", { count: "exact", head: true });
...
export async function toggleStudentBan(studentId, currentStatus) {
  ...
  try {
    ...
    return {
      success: true,
      message: currentStatus ? "تم إلغاء الحظر بنجاح" : "تم حظر الطالب بنجاح",
    };
  } catch (error) {
...
export async function getActivationCodes() {
  ...
  try {
    ...
    if (error) {
      ...
      console.log(
        "Initial fetch error, trying simpler student relation",
        error,
      );
      throw error;
    }
    ...
    return (
      codes.map((code) => {
        ...
        if (code.type === "course") {
          targetTitle =
            courses?.find((c) => c.id === code.target_id)?.title ||
            "كورس محذوف";
        } else if (code.type === "bundle") {
          targetTitle =
            bundles?.find((b) => b.id === code.target_id)?.title ||
            "باقة محذوفة";
        }
    ...
    try {
      ...
      const { data: simpleCodes } = await supabaseRetry
        .from("activation_codes")
        .select("*")
        .order("created_at", { ascending: false });
      return simpleCodes || [];
...
export async function generateCodes({ type, targetId, amount, balanceAmount }) {
  ...
  try {
    ...
    return {
      success: true,
      message: `تم إنشاء ${amount} كود بنجاح!`,
      codes: codesToInsert,
    };
  } catch (error) {
...
export async function getDevices() {
  ...
  try {
    ...
    const studentIds = [
      ...new Set(devices.map((d) => d.student_id).filter(Boolean)),
    ];
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === "23503") {
        ...
        return {
          success: false,
          message: "لا يمكن حذف التصنيف لأنه مستخدم في كورسات",
        };
      }
      throw error;
    }
...
export async function createLesson(
  ...
  try {
    ...
    if (type === "exam") {
      ...
      lessonData.allowed_attempts = parseInt(allowed_attempts) || 0;
    }
    ...
    return {
      success: false,
      message: "خطأ في إضافة الدرس: " + (error.message || ""),
    };
  }
}
...
export async function updateLesson(
  ...
  try {
    ...
    return {
      success: false,
      message: "خطأ في تحديث الدرس: " + (error.message || ""),
    };
  }
}
...
export async function getRecentActivity() {
  ...
  try {
    ...
    // 3. Get recently used codes (Fixed query)
    const { data: recentCodes } = await supabase
      ...
      .select(
        `
                ...
                target_id,
                used_at,
                student:profiles!activation_codes_used_by_student_id_fkey(full_name)
            `,
      )
      .eq("is_used", true)
    ...
    // 6. Get recent lesson completions
    ...
    // Fetch titles for polymorphic codes
    const { data: courses } = await supabase
      .from("courses")
      .select("id, title");
    const { data: bundles } = await supabase
    ...
    const findTitle = (type, id) => {
      ...
      if (type === "bundle") return bundles?.find((b) => b.id === id)?.title;
      return null;
    };
    ...
    recentStudents?.forEach((s) => {
      notifications.push({
        ...
        time: s.created_at,
        icon: "user",
        href: `/admin/students?search=${s.full_name || ""}`,
      });
    });
    ...
    recentCodes?.forEach((c) => {
      const targetName = findTitle(c.type, c.target_id) || "محتوى";
      notifications.push({
        id: `code-${c.code}`,
        type: "code_used",
        title: "كود مستخدم",
        message: `${c.student?.full_name || "طالب"} فعّل ${targetName} باستخدام كود`,
        time: c.used_at,
        icon: "key",
        href: `/admin/activation-codes?search=${c.code}`,
      });
    });
    ...
    recentDevices?.forEach((d) => {
      notifications.push({
        ...
        title: "جهاز جديد",
        message: `سجّل ${d.student?.full_name || "طالب"} دخول من جهاز جديد: ${d.device_name || "غير معروف"}`,
        time: d.created_at,
        icon: "smartphone",
        href: "/admin/devices",
      });
    ...
    recentProgress?.forEach((p) => {
      notifications.push({
        id: `progress-${p.student?.full_name}-${p.lesson?.title}`,
        type: "lesson_completed",
        title: "إكمال درس",
        message: `أتمّ ${p.student?.full_name || "طالب"} مشاهدة درس: ${p.lesson?.title || "غير معروف"}`,
        time: p.updated_at,
        icon: "trophy",
        href: "/admin/students",
      });
...
export async function submitExamResult({
  ...
  const supabase = await createClient();
  try {
    // 1. Get lesson attempt limit
    const { data: lesson, error: lessonError } = await supabase
      .from("lessons")
      .select("allowed_attempts, passing_score")
      .eq("id", lessonId)
      .single();
    ...
    // 3. Enforce limit (if allowed_attempts > 0)
    if (
      lesson.allowed_attempts > 0 &&
      (count || 0) >= lesson.allowed_attempts
    ) {
      return {
        success: false,
        message: "لقد استنفدت جميع المحاولات المسموحة لهذا الامتحان",
      };
    }

    const isPassed =
      (score / (totalScore || 1)) * 100 >= (lesson.passing_score || 50);
    ...
    return { success: true, message: "تم تسجيل النتيجة بنجاح" };
  } catch (error) {
    console.error("Submit exam error:", error);
    return { success: false, message: "خطأ في تسجيل النتيجة" };
  }
}

export async function checkExamAvailability(studentId, lessonId) {
  const supabase = await createClient();
  try {
    const { data: lesson, error: lessonError } = await supabase
      .from("lessons")
    ...
    if (lesson.allowed_attempts === 0) return { allowed: true };
    ...
    const remaining = lesson.allowed_attempts - (count || 0);

    return {
      allowed: remaining > 0,
      remaining,
      attemptsMade: count || 0,
      totalAllowed: lesson.allowed_attempts,
    };
  } catch (error) {
    return { allowed: false, message: "خطأ في التحقق من التوفر" };
  }
}
